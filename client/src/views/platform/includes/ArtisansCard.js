...
const ArtisansCard = () => {
  ...
  useEffect(() => {
    ...
    // blob object representation of raw binary data.
    api_connect
      .get("/auth/fetch-user-profile/" + artisans.profile, {
        responseType: "blob",
      })
      .then((response) => {
        if (response.status === 200) {
  ...
  return (
    <>
      <h2 className="artisans-card-header">Artisans</h2>
